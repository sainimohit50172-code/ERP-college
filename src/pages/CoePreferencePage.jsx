...
import { ClipboardCheck, FileDigit } from 'lucide-react';
...
const cards = [
  {
    title: 'DMC on Student App Management',
    subtitle: 'Manage DMC number series and student app DMC configuration.',
    icon: FileDigit,
    route: '/coe/student-dmc',
  },
  {
    title: 'Marks Entry Preference Management',
    subtitle: 'Configure marks entry preferences and exam form rules.',
    icon: ClipboardCheck,
    route: '/coe/master/exam-form-preferences/settings',
  },
];

export default function CoePreferencePage() {
  return (
    <div className="no-hover-border min-h-[calc(100vh-7rem)] overflow-hidden rounded-[24px] border border-slate-200/80 bg-[linear-gradient(135deg,#f8fafc_0%,#ffffff_55%,#f8fafc_100%)] p-2.5 shadow-[0_18px_45px_rgba(15,23,42,0.06)] sm:p-3 lg:p-4">
      <div className="no-hover-border flex h-full flex-col rounded-[22px] border border-slate-200/70 bg-white/90 p-3 shadow-inner sm:p-4 lg:p-5">
        <div className="mb-8">
          <Breadcrumb items={[{ label: 'Dashboard', to: '/' }, { label: 'COE Master', to: '/settings/coe' }, { label: 'COE Preference' }]} />
          <div className="flex flex-col gap-2">
            <span className="text-sm font-semibold uppercase tracking-[0.24em] text-emerald-600">COE Master</span>
            <h1 className="text-3xl font-semibold tracking-tight text-slate-950">COE Preference</h1>
            <p className="max-w-2xl text-sm text-slate-500">Use these preference cards to manage COE settings across student DMC and marks entry workflows.</p>
          </div>
        </div>
        ...
        <section className="grid gap-4 sm:grid-cols-2">
          {cards.map((card, index) => {
            ...
            return (
              ...
              >
                <div className="rounded-full bg-[#F5F8FC] p-3 text-emerald-600 transition duration-300 group-hover:bg-emerald-600 group-hover:text-white w-fit">
