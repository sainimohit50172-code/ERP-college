    ...
    <div className="space-y-8">
      ...
      <Modal title="Record lecture attendance" isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} footer={<button onClick={handleSubmit(onSubmit)} className="rounded-3xl bg-sky-400 px-5 py-3 text-sm font-semibold text-slate-950 transition hover:bg-sky-300">Save attendance</button>}>
        <form className="grid gap-5 lg:grid-cols-2">
          ...
          <FormField label="Total students"><input type="number" {...register('totalStudents')} className="w-full rounded-3xl border border-white/10 bg-slate-900/80 px-4 py-3 text-slate-100 outline-none focus:border-sky-400" placeholder="58" /></FormField>
          <FormField label="Present"><input type="number" min="0" {...register('present', { required: 'Present count is required' })} className="w-full rounded-3xl border border-white/10 bg-slate-900/80 px-4 py-3 text-slate-100 outline-none focus:border-sky-400" placeholder="55" />{errors.present && <p className="mt-1 text-sm text-rose-400">{errors.present.message}</p>}</FormField>
          <FormField label="Absent"><input type="number" min="0" {...register('absent')} className="w-full rounded-3xl border border-white/10 bg-slate-900/80 px-4 py-3 text-slate-100 outline-none focus:border-sky-400" placeholder="2" /></FormField>
          <FormField label="Late arrivals"><input type="number" min="0" {...register('late')} className="w-full rounded-3xl border border-white/10 bg-slate-900/80 px-4 py-3 text-slate-100 outline-none focus:border-sky-400" placeholder="1" /></FormField>
          <FormField label="Status"><select {...register('status')} className="w-full rounded-3xl border border-white/10 bg-slate-900/80 px-4 py-3 text-slate-100 outline-none focus:border-sky-400"><option value="Completed">Completed</option><option value="Pending">Pending</option><option value="Cancelled">Cancelled</option></select></FormField>
        </form>
      </Modal>
    </div>
  );
