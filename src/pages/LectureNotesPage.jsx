    ...
    <div className="space-y-6">
      ...
      <Modal title="Upload lecture notes" isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} footer={<Button onClick={handleSubmit(onSubmit)} variant="primary" >Upload</Button>}>
        <form className="grid gap-5 lg:grid-cols-2">
          ...
          <FormField label="File type"><select {...register('fileType')} className="w-full rounded-3xl border border-white/10 bg-slate-900/80 px-4 py-3 text-slate-100 outline-none focus:border-sky-400 hover-gradient-border"><option value="PDF">PDF</option><option value="DOCX">Word Document</option><option value="PPTX">Presentation</option><option value="ZIP">Compressed</option></select></FormField>
          ...
          <FormField label="Upload file"><input type="file" {...register('file')} className="w-full rounded-3xl border border-white/10 bg-slate-900/80 px-4 py-3 text-slate-100 outline-none focus:border-sky-400 hover-gradient-border" /></FormField>
        </form>
      </Modal>
    </div>
  );
