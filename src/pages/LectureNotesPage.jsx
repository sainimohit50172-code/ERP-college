...
export default function LectureNotesPage() {
  const { data, isLoading } = useResourceList('lectureNotes', { page: 1, pageSize: 200 });
    ...
    <div className="space-y-8">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <div className="rounded-[28px] border border-white/10 bg-slate-900/80 p-6 shadow-sm">
          <p className="text-sm uppercase tracking-[0.24em] text-slate-400">Total notes</p>
          <p className="mt-4 text-3xl font-semibold text-white">{totalNotes}</p>
        </div>
        <div className="rounded-[28px] border border-white/10 bg-slate-900/80 p-6 shadow-sm">
        ...
        <div className="rounded-[28px] border border-white/10 bg-slate-900/80 p-6 shadow-sm">
          <p className="text-sm uppercase tracking-[0.24em] text-slate-400">Total downloads</p>
          <p className="mt-4 text-3xl font-semibold text-white">{totalDownloads}</p>
        </div>
      </div>
      ...
      <div className="rounded-[32px] border border-white/10 bg-slate-900/80 p-6 shadow-soft">
        <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
          <div>
            <h2 className="text-xl font-semibold text-white">Materials library</h2>
            <p className="text-sm text-slate-400">Upload lecture notes, manage versions and track usage statistics.</p>
          </div>
          <button onClick={() => setIsModalOpen(true)} className="inline-flex items-center gap-2 rounded-3xl bg-sky-400 px-4 py-3 text-sm font-semibold text-slate-950 transition hover:bg-sky-300"><FaPlus /> Upload notes</button>
        </div>
