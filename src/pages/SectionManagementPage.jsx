    ...
    <div className="space-y-6">
      ...
      <Modal title="Add new section" isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} footer={<button onClick={handleSubmit(onSubmit)} className="rounded-3xl bg-sky-400 px-5 py-3 text-sm font-semibold text-slate-950 transition hover:bg-sky-300">Create section</button>}>
        <form className="grid gap-5 lg:grid-cols-2">
          ...
          <FormField label="Course"><select {...register('course', { required: 'Course is required' })} className="w-full rounded-3xl border border-white/10 bg-slate-900/80 px-4 py-3 text-slate-100 outline-none focus:border-sky-400"><option value="BCA">BCA</option><option value="MBA">MBA</option><option value="BSc Biology">BSc Biology</option><option value="BA English">BA English</option><option value="BCom">BCom</option></select>{errors.course && <p className="mt-1 text-sm text-rose-400">{errors.course.message}</p>}</FormField>
          ...
          <FormField label="Class advisor"><input type="text" {...register('advisor', { required: 'Advisor is required' })} className="w-full rounded-3xl border border-white/10 bg-slate-900/80 px-4 py-3 text-slate-100 outline-none focus:border-sky-400" placeholder="Dr. Priya Menon" />{errors.advisor && <p className="mt-1 text-sm text-rose-400">{errors.advisor.message}</p>}</FormField>
