...
export default function SyllabusUploadPage() {
  ...
  return (
    <div className="space-y-8">
      ...
      <Modal title="Upload syllabus" isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} footer={<button onClick={handleSubmit(onSubmit)} className="rounded-3xl bg-sky-400 px-5 py-3 text-sm font-semibold text-slate-950 transition hover:bg-sky-300">Save syllabus</button>}>
        <form className="grid gap-5 lg:grid-cols-2">
          ...
          <FormField label="Subject code"><input type="text" {...register('code', { required: 'Code is required' })} className="w-full rounded-3xl border border-white/10 bg-slate-900/80 px-4 py-3 text-slate-100 outline-none focus:border-sky-400" placeholder="CS501" />{errors.code && <p className="mt-1 text-sm text-rose-400">{errors.code.message}</p>}</FormField>
          ...
          <FormField label="Version"><input type="text" {...register('version')} className="w-full rounded-3xl border border-white/10 bg-slate-900/80 px-4 py-3 text-slate-100 outline-none focus:border-sky-400" placeholder="1.0" /></FormField>
          <FormField label="Number of topics"><input type="number" {...register('topics', { required: 'Topics count is required' })} className="w-full rounded-3xl border border-white/10 bg-slate-900/80 px-4 py-3 text-slate-100 outline-none focus:border-sky-400" placeholder="12" />{errors.topics && <p className="mt-1 text-sm text-rose-400">{errors.topics.message}</p>}</FormField>
