...
export default function PracticalMarksPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <Modal title="Enter practical marks" isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} footer={<button onClick={handleSubmit(onSubmit)} className="rounded-3xl bg-sky-400 px-5 py-3 text-sm font-semibold text-slate-950 transition hover:bg-sky-300 hover-gradient-border">Save marks</button>}>
        <form className="grid gap-5 lg:grid-cols-2">
          ...
          <FormField label="Subject"><input type="text" {...register('subject', { required: 'Subject is required' })} className="w-full rounded-3xl border border-white/10 bg-slate-900/80 px-4 py-3 text-slate-100 outline-none focus:border-sky-400 hover-gradient-border" placeholder="Data Structures Lab" />{errors.subject && <p className="mt-1 text-sm text-rose-400">{errors.subject.message}</p>}</FormField>
          <FormField label="Experiment 1 (out of 10)"><input type="number" min="0" max="10" {...register('experiment1')} className="w-full rounded-3xl border border-white/10 bg-slate-900/80 px-4 py-3 text-slate-100 outline-none focus:border-sky-400 hover-gradient-border" placeholder="9" /></FormField>
          <FormField label="Experiment 2 (out of 10)"><input type="number" min="0" max="10" {...register('experiment2')} className="w-full rounded-3xl border border-white/10 bg-slate-900/80 px-4 py-3 text-slate-100 outline-none focus:border-sky-400 hover-gradient-border" placeholder="8" /></FormField>
          <FormField label="Experiment 3 (out of 10)"><input type="number" min="0" max="10" {...register('experiment3')} className="w-full rounded-3xl border border-white/10 bg-slate-900/80 px-4 py-3 text-slate-100 outline-none focus:border-sky-400 hover-gradient-border" placeholder="9" /></FormField>
          <FormField label="Journal (out of 10)"><input type="number" min="0" max="10" {...register('journal')} className="w-full rounded-3xl border border-white/10 bg-slate-900/80 px-4 py-3 text-slate-100 outline-none focus:border-sky-400 hover-gradient-border" placeholder="8" /></FormField>
          <FormField label="Viva (out of 10)"><input type="number" min="0" max="10" {...register('viva')} className="w-full rounded-3xl border border-white/10 bg-slate-900/80 px-4 py-3 text-slate-100 outline-none focus:border-sky-400 hover-gradient-border" placeholder="8" /></FormField>
