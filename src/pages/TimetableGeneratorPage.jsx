    ...
    <div className="space-y-8">
      ...
      <Modal title="Generate new timetable" isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} footer={<button onClick={handleSubmit(onSubmit)} className="rounded-3xl bg-sky-400 px-5 py-3 text-sm font-semibold text-slate-950 transition hover:bg-sky-300">Generate</button>}>
        <form className="grid gap-5 lg:grid-cols-2">
          ...
          <FormField label="Section"><select {...register('section', { required: 'Section is required' })} className="w-full rounded-3xl border border-white/10 bg-slate-900/80 px-4 py-3 text-slate-100 outline-none focus:border-sky-400"><option value="A">A</option><option value="B">B</option><option value="C">C</option></select>{errors.section && <p className="mt-1 text-sm text-rose-400">{errors.section.message}</p>}</FormField>
          <FormField label="Conflict resolution"><select {...register('conflictResolution')} className="w-full rounded-3xl border border-white/10 bg-slate-900/80 px-4 py-3 text-slate-100 outline-none focus:border-sky-400"><option value="Auto">Automatic</option><option value="Manual">Manual Review</option><option value="Strict">Strict (No conflicts)</option></select></FormField>
        </form>
      </Modal>
    </div>
  );
