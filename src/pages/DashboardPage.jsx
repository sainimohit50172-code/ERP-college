    ...
    <div className="space-y-8">
      <div className="rounded-[32px] border border-slate-200/70 bg-white/95 p-6 shadow-[0_35px_80px_rgba(15,23,42,0.08)]">
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div>
            <p className="text-sm uppercase tracking-[0.24em] text-emerald-600">Super Admin Dashboard</p>
            <h1 className="mt-3 text-4xl font-semibold text-slate-950">Executive overview</h1>
            <p className="max-w-2xl text-slate-500">Monitor admissions, operations, finance, attendance, and student success from one premium experience.</p>
          </div>
          <div className="grid gap-3 sm:grid-cols-3">
            {kpiHighlights.map((item) => (
              <div key={item.title} className={`rounded-3xl px-5 py-4 shadow-sm ${item.accent}`}>
      ...
      <div className="grid gap-6 xl:grid-cols-[1.6fr_1fr]">
        ...
        <section className="rounded-[32px] border border-slate-200/70 bg-white/95 p-6 shadow-[0_35px_80px_rgba(15,23,42,0.08)]">
          <div className="flex items-center justify-between gap-4">
            <div>
              <h2 className="text-xl font-semibold text-slate-950">Admissions performance</h2>
              <p className="text-sm text-slate-500">Weekly funnel metrics and campaign performance.</p>
            </div>
            <button className="rounded-2xl bg-emerald-50 px-4 py-2 text-sm font-semibold text-emerald-700 transition hover:bg-emerald-100">Export</button>
          </div>
