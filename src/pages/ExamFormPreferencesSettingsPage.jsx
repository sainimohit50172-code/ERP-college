...
function PreferenceFields({ register, errors }) {
  ...
  return <div className="grid gap-4 sm:grid-cols-2">{fields.map(([name, label]) => <label key={name} htmlFor={`exam-preference-${name}`}><span className="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-slate-600">{label}</span><input id={`exam-preference-${name}`} type="number" min="1" {...register(name, { required: `${label} is required`, valueAsNumber: true, min: { value: 1, message: `${label} must be selected` } })} className="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm outline-none focus:border-blue-500" />{errors[name] && <span className="mt-1 block text-xs text-rose-600">{errors[name].message}</span>}</label>)}<label htmlFor="exam-preference-status"><span className="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-slate-600">Status</span><select id="exam-preference-status" {...register('status', { required: 'Status is required' })} className="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm outline-none focus:border-blue-500"><option>Active</option><option>Inactive</option><option>Draft</option></select>{errors.status && <span className="mt-1 block text-xs text-rose-600">{errors.status.message}</span>}</label></div>;
}

export default function ExamFormPreferencesSettingsPage() {
  ...
  const toggleSettings = (key, value) => setSettings((current) => ({ ...current, [key]: value }));
  ...
  const openEdit = (item) => { reset({ academicSessionId: item.academicSessionId, instituteId: item.instituteId, courseId: item.courseId, programId: item.programId, semesterId: item.semesterId, examTypeId: item.examTypeId, status: item.status }); setEditor({ item }); };
  ...
  const columns = useMemo(() => [
    ...
    { key: 'status', label: 'Status', render: (value, row) => <button type="button" onClick={() => toggleStatus(row)} title="Toggle status"><StatusBadge status={value} /></button> },
    { key: 'action', label: 'Action', sortable: false, render: (_value, row) => <div className="flex items-center justify-end gap-1"><button type="button" title="View" onClick={() => setView(row)} className="rounded-lg p-2 text-slate-500 hover:bg-slate-100"><Eye className="h-4 w-4" /></button><button type="button" title="Edit" onClick={() => openEdit(row)} className="rounded-lg p-2 text-slate-500 hover:bg-slate-100"><Pencil className="h-4 w-4" /></button><button type="button" title="Delete" onClick={() => setRemove(row)} className="rounded-lg p-2 text-rose-500 hover:bg-rose-50"><Trash2 className="h-4 w-4" /></button></div> },
  ], [rows]);
