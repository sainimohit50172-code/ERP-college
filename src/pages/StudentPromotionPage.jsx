...
export default function StudentPromotionPage() {
  ...
  return (
    <div className="space-y-8">
      ...
      <Modal title="Process student promotion" isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} footer={<button onClick={handleSubmit(onSubmit)} className="rounded-3xl bg-sky-400 px-5 py-3 text-sm font-semibold text-slate-950 transition hover:bg-sky-300 hover-gradient-border">Process promotion</button>}>
        <form className="grid gap-5 lg:grid-cols-2">
          ...
          <FormField label="Current semester"><input type="text" {...register('currentSem', { required: 'Current semester is required' })} className="w-full rounded-3xl border border-white/10 bg-slate-900/80 px-4 py-3 text-slate-100 outline-none focus:border-sky-400 hover-gradient-border" placeholder="5" />{errors.currentSem && <p className="mt-1 text-sm text-rose-400">{errors.currentSem.message}</p>}</FormField>
          <FormField label="Next semester"><input type="text" {...register('nextSem', { required: 'Next semester is required' })} className="w-full rounded-3xl border border-white/10 bg-slate-900/80 px-4 py-3 text-slate-100 outline-none focus:border-sky-400 hover-gradient-border" placeholder="6" />{errors.nextSem && <p className="mt-1 text-sm text-rose-400">{errors.nextSem.message}</p>}</FormField>
          <FormField label="CGPA"><input type="number" step="0.01" {...register('cgpa')} className="w-full rounded-3xl border border-white/10 bg-slate-900/80 px-4 py-3 text-slate-100 outline-none focus:border-sky-400 hover-gradient-border" placeholder="3.45" /></FormField>
          <FormField label="Number of backlogs"><input type="number" min="0" {...register('backlog')} className="w-full rounded-3xl border border-white/10 bg-slate-900/80 px-4 py-3 text-slate-100 outline-none focus:border-sky-400 hover-gradient-border" placeholder="0" /></FormField>
          <FormField label="Eligibility"><select {...register('eligibility')} className="w-full rounded-3xl border border-white/10 bg-slate-900/80 px-4 py-3 text-slate-100 outline-none focus:border-sky-400 hover-gradient-border"><option value="Pass">Pass</option><option value="Conditional Pass">Conditional Pass</option><option value="Fail">Fail</option></select></FormField>
          <FormField label="Promotion status"><select {...register('promotionStatus')} className="w-full rounded-3xl border border-white/10 bg-slate-900/80 px-4 py-3 text-slate-100 outline-none focus:border-sky-400 hover-gradient-border"><option value="Promoted">Promoted</option><option value="Conditional">Conditional</option><option value="Not Promoted">Not Promoted</option></select></FormField>
        </form>
      </Modal>
    </div>
  );
