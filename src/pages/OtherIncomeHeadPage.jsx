...
export default function OtherIncomeHeadPage() {
  ...
  const { data, isLoading } = useResourceList('other-income-heads', { page: 1, pageSize: 100 });
  const createResource = useCreateResource('other-income-heads');
  const updateResource = useUpdateResource('other-income-heads');
  const deleteResource = useDeleteResource('other-income-heads');
  ...
  const handleSubmit = async (event) => {
    ...
    if (!formValues.name.trim() || !formValues.code.trim()) {
      toast.error('Income Head and Code are required.');
      return;
    }
    ...
    try {
      if (editItem) {
        ...
        toast.success('Other income head updated.');
      } else {
        await createResource.mutateAsync(payload);
        toast.success('Other income head created.');
      }
      closeForm();
    } catch (error) {
      toast.error(error?.message || 'Could not save other income head.');
    }
  };

  const handleDelete = async (item) => {
    if (!window.confirm(`Delete ${item.name || 'this income head'}?`)) return;

    try {
      await deleteResource.mutateAsync(item.id);
      toast.success('Other income head deleted.');
    } catch (error) {
      toast.error(error?.message || 'Could not delete other income head.');
    }
  };
  ...
  return (
    <div className="space-y-6 px-4 pb-6 sm:px-0">
      <div className="rounded-[24px] border border-slate-200/70 bg-white/95 p-4 shadow-sm sm:p-6">
        ...
        <div className="flex flex-col gap-4 pb-2 pt-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex min-w-0 items-center gap-1.5">
            <h1 className="truncate text-[clamp(1.7rem,3vw,2.15rem)] font-medium tracking-tight text-slate-950">Other Income Head</h1>
            <span className="truncate text-lg font-medium text-slate-900">| Other Income Head Data</span>
          </div>
          <button
            type="button"
            onClick={openCreate}
        ...
        <div className={`px-0 pb-10 text-center ${showTable ? 'pt-8' : 'min-h-[32rem] pt-36'}`}>
          {!showTable ? null : isLoading ? (
            ...
            <div className="mx-0 max-w-none overflow-x-auto border border-slate-200 bg-white text-left">
      ...
      <Modal
        isOpen={showForm}
        onClose={closeForm}
        title={editItem ? 'Edit Other Income Head' : 'New Other Income Head'}
        footer={(
          <>
            ...
            <button type="submit" form="other-income-head-form" className="inline-flex items-center gap-2 rounded-md bg-[#0a2e1a] px-4 py-2 text-sm font-semibold text-white hover:bg-[#05331e] focus:outline-none focus:ring-2 focus:ring-[#0a2e1a]/30">
