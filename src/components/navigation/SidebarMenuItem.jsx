...
export default function SidebarMenuItem({ item, isExpanded, onToggle, onNavigate, activePath }) {
  ...
  return (
    <Link
      to={item.to || '/'}
      onClick={onNavigate}
      className={`flex cursor-pointer items-center gap-2 rounded-xl px-3 py-2 text-sm transition ${isActive ? 'bg-emerald-500/20 text-white' : 'text-slate-300 hover:bg-white/10 hover:text-white'}`}
    >
      {Icon ? <Icon className="h-4 w-4" /> : null}
      <span>{item.label}</span>
